...
const getProfileProjects = async (after: string) => {
  ...
  const response = await fetch('https://www.behance.net/v3/graphql?client_id=BehanceWebSusi1', {
    method: 'POST',
    headers: {
...
export const GET: APIRoute = (async () => {
  ...
  try {
    ...
    const sanitisedPhotographyProjects: SanitisedBehancePhotographyProject[] = allProjects
      .filter((project) => {
        return project.fields.some((field) => field.id === PHOTOGRAPHY_FIELD_ID);
      })
      .map((project) => {
